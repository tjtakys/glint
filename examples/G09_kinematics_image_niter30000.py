...
obsfits   = str(data_dir / f"B6_concat_CII_dv{dv}km_rob-0.5_clean.image.pbcor.cutout.-250+300km.fits")
noisefits = str(data_dir / f"B6_concat_CII_dv{dv}km_rob-0.5_clean.noise.pbcor.cutout.-250+300km.fits")
...
spec_res_sgm_kms = dv / (2 * np.sqrt(2 * np.log(2)))  # 25 km/s channel width -> sigma
...
# model作成時は速度で指定
vchan_kms = np.arange(-250, 301, dv)
...
niter = 30000
chain, logprob, sampler = run_emcee(logprob_fn=logprob, x0=x0, lb=lb, ub=ub, nwalkers=4*len(lb), burnin=2000, production=niter, ncpu=24, init_frac=0.2, skip_initial_state_check=False)


savename = f"./mcmc_{niter}_chain_G09_{dv}_cube_image.npz"
np.savez(savename,
         chain=chain,
         logprob=logprob,
...
fig.savefig(f"./figure/corner_{niter}_G09_{dv}_lens_fix.pdf", dpi=300, bbox_inches="tight")
# plt.show()

print(f"\nSaved corner plot: ./figure/corner_{niter}_G09_{dv}_lens_fix.pdf")
...
ax.set_xlim(0, 3.5)
ax.set_ylim(0, 600)
fig.savefig(f"./figure/rotation_curve_mcmc_{niter}_G09_{dv}_lens_fix.pdf", dpi=300, bbox_inches="tight")
print(f"Saved to ./figure/rotation_curve_mcmc_{niter}_G09_{dv}_lens_fix.pdf")
# plt.show()
...
fig.savefig(f"./figure/channel_maps_mcmc_{niter}_G09_{dv}_lens_fix.pdf", dpi=300, bbox_inches="tight")
print(f"Saved to ./figure/channel_maps_mcmc_{niter}_G09_{dv}_lens_fix.pdf")
# plt.tight_layout()
